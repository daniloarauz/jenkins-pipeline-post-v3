    ...
    stages {
        stage('Create  directory for the WEB Application')
        {
            steps{
                ...
                sh 'rm -rf /var/lib/jenkins/tomcat-web'
                //Create the directory
                sh 'mkdir /var/lib/jenkins/tomcat-web'
        ...
        stage('Drop the Apache Tomcat Docker container'){
            ...
            sh 'docker rm -f /var/lib/jenkins/tomcat1'
            }
        }
        stage('Create the Tomcat container') {
            steps {
            echo 'Creating the container...'
            sh 'docker run -dit --name tomcat1 -p 9090:8080  -v /var/lib/jenkins/tomcat-web:/usr/local/tomcat/webapps tomcat:9.0'
            }
        }
        stage('Copy the web application to the container directory') {
            steps {
                echo 'Creating the shopping folder in the container'
                sh 'mkdir /var/lib/jenkins/tomcat-web/shopping'
                echo 'Copying web application...'             
                sh 'cp -r shopping/* /var/lib/jenkins/tomcat-web/shopping'
            }
        }
    }
